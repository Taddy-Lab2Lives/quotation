...
export async function generatePDF(
  ...
  const vietText = (text: string): string => {
    // This ensures proper Vietnamese character encoding
    return text;
  };

  // Set font - use default Helvetica which has better Unicode support
  ...
  // Cost Breakdown Section using autoTable for better formatting
  checkNewPage(60);
  pdf.setFillColor(245, 245, 245);
  pdf.rect(margin, yPosition, contentWidth, 8, 'F');
  pdf.setFontSize(12);
  ...
  const costHeaders = language === 'vi' 
    ? ['Hạng mục', 'Chi phí']
    : ['Item', 'Cost'];

  const costRows = [
    [
      vietText(language === 'vi' ? 'Chi phí phần cứng' : 'Hardware Cost'),
      formatCurrency(costs.hardwareCost, language)
    ],
    [
      vietText(language === 'vi' ? 'Chi phí phần mềm (mỗi năm)' : 'Software Cost (per year)'),
      formatCurrency(costs.softwareCostPerYear, language)
    ],
    [
      vietText(language === 'vi' ? 'Chi phí lắp đặt (mỗi cửa hàng)' : 'Installation Cost (per store)'),
      formatCurrency(costs.installationCostPerStore, language)
    ],
    [
      vietText(language === 'vi' ? 'Dịch vụ thiết lập (mỗi cửa hàng)' : 'Setup Service (per store)'),
      formatCurrency(costs.setupServicePerStore, language)
    ]
  ];
